...
export const TOKEN_COMPRESS_MAP: Record<string, string> = {
  // ─── Navigation ──────────────────────────────────
  'navigation':       '导航',
  'navigate':         '导航',
  'nav':              '导航',
 ...
 */
function shouldPreserve(val: unknown): boolean {
 ...
 */
export function applyChineseCompression(obj: unknown): CompressionResult {
  let replacements = 0;
  ...
  const walk = (node: unknown): unknown => {
    ...
    for (const key of Object.keys(node as Record<string, unknown>)) {
      ...
      result[newKey] = walk((node as Record<string, unknown>)[key]);
    }
    return result;
  };
 ...
 */
export function compressForTokens(text: string): string {
  if (!text) return '';
  return text
    .replace(/\s*\n\s*/g, '\n')
    .replace(/ {2,}/g, ' ')
