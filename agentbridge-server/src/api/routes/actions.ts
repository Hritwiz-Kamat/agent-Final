...
import { validateBody, actionsSchema, executeSchema, type ActionsInput, type ExecuteInput } from '../middleware/validation.js';
...
actionsRouter.post('/', validateBody(actionsSchema), async (req, res) => {
  const { url } = req.body as ActionsInput;
...
actionsRouter.post('/execute', validateBody(executeSchema), async (req, res) => {
  const { url, selector, action, value } = req.body as ExecuteInput;
