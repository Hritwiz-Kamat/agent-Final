...
import { validateBody, extractSchema, type ExtractInput } from '../middleware/validation.js';
...
extractRouter.post('/', validateBody(extractSchema), async (req, res) => {
  const { url, format, chineseCompression, waitUntil } = req.body as ExtractInput;
