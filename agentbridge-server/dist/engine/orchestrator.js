...
import { formatMCP, formatRaw, formatTextOnly, } from './mcp-formatter.js';
...
export async function extractPage(url, options = {}) {
    ...
    const { chineseCompression = config.chineseCompression, waitUntil = 'networkidle0', navigationTimeout = config.navigationTimeout, useReadability = config.useReadability, } = options;
    ...
    try {
        ...
        log.debug({ elementsExtracted: extractionData.stats?.elementsCounted }, 'Extraction complete');
        // 5. Inject and run action mapper
        await page.evaluate(scripts.actionMapperScript);
        const tools = await page.evaluate(() => {
            const mapper = window.__agentBridge_actionMapper;
            if (!mapper)
                throw new Error('ActionMapper not loaded');
        ...
        if (chineseCompression) {
            ...
            log.info({ mcpReplacements: mcpRep, rawReplacements: rawRep }, 'Chinese compression applied');
        }
        const elapsed = Date.now() - startTime;
        log.info({ url, elapsed, toolCount: tools.length, tokenSavings: extractionData.stats?.tokenSavings }, 'Extraction complete');
        return output;
    }
    catch (err) {
 ...
 */
export async function listActions(url) {
    const page = await browserPool.acquirePage();
    try {
        await page.goto(url, {
 ...
 */
export async function executeAction(url, selector, action, value) {
    const page = await browserPool.acquirePage();
    try {
        await page.goto(url, {
            waitUntil: 'networkidle0',
